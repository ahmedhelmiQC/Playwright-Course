...
test("user Buy ZARA COAT",async({page})=>{
    ...
    //await rigisterpage.fillRegisterForm("taymia.somia@gmail.com","Ahmed@123");

    await loginpage.fillLoginForm("taymia.somia@gmail.com","Ahmed@123");
    await loginpage.handeltabs();

    await addtocard.addtocart();

    await payment.fillPaymentForm();
     expect(payment.successMessage).toContainText("Click To Download Order Details in CSV");
